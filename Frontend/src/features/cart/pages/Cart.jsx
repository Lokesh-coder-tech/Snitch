...
const Cart = () => {
  ...
  // NOTE: Ensure handleDecrementCartItem and handleRemoveCartItem exist in your useCart hook!
  const {
    handleGetCart,
  ...
  const getVariantDetails = (product, variantId) => {
    ...
    // Find the specific variant inside the array if variants is populated
  ...
  async function handleCheckout() {
    ...
    const razorpayInstance = new window.Razorpay(options); // Assumed window object if using script tag
          ...
          <div className="flex flex-col lg:flex-row gap-12 lg:gap-20 items-start">
            ...
            <div className="w-full lg:w-[65%]">
              ...
              <div className="flex flex-col gap-6">
                {cart.items.map((item) => {
                  // SAFELY EXTRACT IDs TO PREVENT 400 ERRORS
                  ...
                  // Use quantity directly from Redux state
                  const qty = item.quantity ?? 1;

                  const displayPrice =
                    item.price ?? variantDetail?.price ?? item.product?.price;
                        ...
                        <div className="flex items-center justify-between flex-wrap gap-4">
                          ...
                          >
                            <button
                              ...
                              disabled={qty <= 1}
                              className={`w-9 h-9 flex items-center justify-center text-sm font-light transition-colors ${qty <= 1 ? "opacity-30 cursor-not-allowed" : "hover:opacity-60"}`}
                              style={{
                                color: tokens.onSurface,
                            ...
                            <button
                              ...
                              className={`w-9 h-9 flex items-center justify-center text-sm font-light transition-colors ${stock !== undefined && qty >= stock ? "opacity-30 cursor-not-allowed" : "hover:opacity-60"}`}
                              style={{
                                color: tokens.onSurface,
